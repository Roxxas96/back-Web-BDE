...
export async function getGoodiesPicture(fastify: FastifyInstance, goodiesPictureId: string) {
  if (!goodiesPictureId) {
    throw fastify.httpErrors.badRequest("Invalid goodies");
  }
...
export async function deleteGoodiesPicture(
  ...
  const goodies = await fastify.prisma.goodies.getGoodies(
    goodiesId
  );

  await fastify.minio.goodiesPicture.getGoodiesPicture(goodies.imageId);

  return await fastify.minio.goodiesPicture.deleteGoodiesPicture(goodies.imageId);
}
